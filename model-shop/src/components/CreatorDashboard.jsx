import { mockCreatorStats, mockModels } from '../data/mockModels';
    ...
    <div className="text-white space-y-8 max-w-7xl mx-auto py-6">
      ...
      <div className="flex justify-between items-center bg-slate-900 p-6 border border-slate-800 rounded-2xl">
        <div>
          ...
          <p className="text-slate-400 text-sm">Managing listings for <span className="text-indigo-400 font-semibold">{mockCreatorStats.creatorHandle}</span></p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl">
          ...
          <p className="text-3xl font-extrabold text-emerald-400 mt-2">₹{mockCreatorStats.totalEarnings}</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl">
        ...
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl">
          <p className="text-slate-400 text-xs font-bold uppercase tracking-wider">Listing Views</p>
          <p className="text-3xl font-extrabold text-indigo-400 mt-2">{mockCreatorStats.totalViews}</p>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-slate-800">
              {mockModels.slice(0, 3).map((model) => (
                <tr key={model.id} className="hover:bg-slate-800/30">
                  <td className="p-3 font-medium text-white">{model.title}</td>
                  <td className="p-3"><span className="bg-indigo-500/10 text-indigo-400 px-2 py-0.5 rounded text-xs">{model.category}</span></td>
                  <td className="p-3 text-emerald-400 font-semibold">{model.isFree ? 'Free' : `₹${model.price}`}</td>
                  <td className="p-3">{model.downloads}</td>
                </tr>
              ))}
            </tbody>
          </table>
